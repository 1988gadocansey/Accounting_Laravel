...
class CrudViewCommand extends Command {
	 ...
	 * @var string
	 */
	protected $signature = 'ropat-crud:view
...
EOD;
	...
	protected function createField($item) {
		$item['type'] = $this->checkType($item['type']);
	...
	protected function checkType($text) {
		$text = strtolower($text);
		if (str_contains($text, "blob")) {
			return "blob";
		}
		...
		if (str_contains($text, "datetime")) {
			return "datetime";
		}
		if (str_contains($text, "date")) {
			return "date";
		}
		//if nothing matches just return text as it is
		return $text;
	}
