...
namespace Ropat\CrudGenerator\Commands;
...
class CrudCommand extends Command {
	 ...
	 * @var string
	 */
	protected $signature = 'ropat-crud:generate
	...
	public function handle() {
		...
		$name = studly_case(str_slug($name));
		...
		if ($this->option('fields')) {
			...
			$this->call('ropat-crud:controller', ['name' => $controllerNamespace . $name . 'Controller', '--crud-name' => $name, '--model-name' => $modelName, '--view-path' => $viewPath, '--required-fields' => $requiredFields, '--route-group' => $routeGroup]);
			$this->call('ropat-crud:model', ['name' => 'Models/' . $modelName, '--fillable' => $fillable, '--table' => $tableName]);
			//$this->call('ropat-crud:migration', ['name' => $migrationName, '--schema' => $fields, '--pk' => $primaryKey]);
			$this->call('ropat-crud:view', ['name' => $viewName, '--fields' => $fields, '--view-path' => $viewPath, '--route-group' => $routeGroup]);
		...
		if (file_exists($routeFile) && (strtolower($this->option('route')) === 'yes')) {
			...
			if (\App::VERSION() >= '5.2') {
				$isAdded = File::append($routeFile,
					...
					. "\nRoute::any('" . $routeName . "/search', '" . $controller . "@search');"
					. "\n});"
				);
			} else {
				$isAdded = File::append($routeFile, "\nRoute::resource('" . $routeName . "', '" . $controller . "');" . "\nRoute::any('" . $routeName . "/search', '" . $controller . "@search');");
